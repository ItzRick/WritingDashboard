    ...
    def textDoi(self, doi):
        '''
            ...
            Attributes:
                ...
                isSuccessful: Boolean to indicate if the text has been successfully extracted from the pdf file.
            Arguments:
                self: The current class object.
        ...
        if downloadDoi(doi, filePath):
            isSuccessful, text = getPDFText(filePath)
            # If the retrieval of text is not successful, set the text to the empty string:
            if not isSuccessful: 
                text = ''
