...
def testAvgEmptyUser(testClient, initDatabaseEmpty):
    '''
        ...
        Attributes: 
            file: file instance 
            user: user instance we test this for.
            data: The data we are trying to test the getAvgScore with.
            response: Response of the get request.
            access_token: the access token
        Arguments:
    ...
    assert response.data == b'No files for user'
    
def testAvgNoScores(testClient, initDatabaseEmpty):
    '''
        This test checks whether we get an error when we ask the average scores for a user with files, but without scores.
        Attributes: 
            data: The data we are trying to test the getAvgScore with.
            response: Response of the get request.
            access_token: the access token
    ...
    file = Files(path='', filename='', date=datetime(2018, 1, 1), userId = user.id, courseCode = '', id=1)
    db.session.add(file)
    db.session.commit()
    ...
    # Check if we get the correct status code
    assert response.status_code == 400
    assert response.data == b'No files with scores for user'
