...
class BaseFeedback:
    ...
    def uploadToDatabase(self):
        ...
        '''
        # Upload the scores:
        setScoreDB(self.fileId, self.scoreStyle, self.scoreCohesion, self.scoreStructure, self.scoreIntegration, int(current_app.config['FEEDBACKVERSION']))
        ...
        if len(self.explanations) > 0:
            ...
            if len(explanationIds) == len(self.explanations):
                for idexp, (X1, Y1, X2, Y2, explType, expl, mistake, replacements) in enumerate(self.explanations):
                    self.uploadExplanation(X1, Y1, X2, Y2, explanationIds[idexp], explType, expl, mistake, replacements, int(current_app.config['FEEDBACKVERSION']))
            else:
                ...
                for (X1, Y1, X2, Y2, explType, expl, mistake, replacements) in self.explanations:
                    self.uploadExplanation(X1, Y1, X2, Y2, -1, explType, expl, mistake, replacements, int(current_app.config['FEEDBACKVERSION']))
